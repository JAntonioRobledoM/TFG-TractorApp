    ...
    <div class="container mx-auto p-6">
      ...
      <div class="mt-8 bg-white shadow-lg overflow-hidden rounded-lg border-t-4 border-yellow-600">
        <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
          <h3 class="text-lg leading-6 font-medium text-green-800 flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-yellow-600" viewBox="0 0 24 24"
              fill="currentColor">
              <path
          ...
          <Link :href="route('users.tractors', user.id)"
        ...
        <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
          ...
          <div class="flex space-x-2">
            ...
            <Link :href="route('users.assign-tractors', user.id)"
              class="text-yellow-600 hover:text-yellow-800 hover:bg-yellow-50 px-3 py-2 rounded-lg transition duration-200 flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
              <path fill-rule="evenodd"
        ...
        <div v-if="user.tractors && user.tractors.length" class="border-t border-green-200">
          <div v-for="tractor in user.tractors.slice(0, 3)" :key="tractor.id"
            class="px-4 py-4 sm:px-6 border-b border-green-100 last:border-b-0 hover:bg-green-50 transition duration-200">
            <div class="flex justify-between items-center">
              ...
              <Link :href="route('tractors.show', tractor.id)"
      ...
      <div class="mt-8 bg-white shadow-lg overflow-hidden rounded-lg border-t-4 border-green-600">
        <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
          ...
          <Link :href="route('users.listings', user.id)"
            class="text-green-600 hover:text-green-800 hover:bg-green-50 px-3 py-2 rounded-lg transition duration-200">
          Ver todos los anuncios
          </Link>
        </div>
        <div v-if="user.listings && user.listings.length" class="border-t border-green-200">
          <div v-for="listing in user.listings.slice(0, 3)" :key="listing.id"
            class="px-4 py-4 sm:px-6 border-b border-green-100 last:border-b-0 hover:bg-green-50 transition duration-200">
            <div class="flex justify-between items-center">
              <div>
                <p class="text-sm font-medium text-gray-900">
